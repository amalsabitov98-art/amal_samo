...
# Тесты пересобирают наполнение генератором и db/seed.sql не читают, а в
# боевую базу заливается именно он. Один раз это уже разошлось и в проде
# не оказалось каталога — теперь сверяем явно.
echo "Сверяю db/seed.sql с генератором…"
if ! python3 tools/check-seed.py; then
  echo "(тесты продолжатся, но перед деплоем это надо поправить)"
fi
echo

# Осиротевший перевод — ключ, которому в данных больше ничего не
# соответствует: русский текст переписали, а ключ нет, и строка молча
# перестала переводиться. Непереведённые строки — только предупреждение.
# Хранилище фотографий. Ветка KV живёт ТОЛЬКО в воркере: демо-режим идёт
# мимо него целиком, и сломайся она — все дымовые тесты остались бы
# зелёными. Бэкенд для этого поднимать не нужно, воркер импортируется
# модулем, а хранилища подменяются поддельными.
echo "Проверяю хранилище фотографий…"
if ! node test/media-test.mjs; then
  echo "(ошибка в хранилище фотографий)"
fi
echo

echo "Сверяю переводы наполнения…"
if ! python3 tools/check-i18n.py; then
  echo "(тесты продолжатся, но перед деплоем это надо поправить)"
fi
echo
