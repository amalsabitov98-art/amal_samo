    ...
    uploadMedia: function (blob, kind) {
      ...
      return fetch(API_BASE + "/api/admin/media?kind=" + encodeURIComponent(kind || ""), {
        ...
        return res.json().catch(function () { return {}; }).then(function (data) {
          // 404 здесь означает не «файл не найден», а «на сервере ещё нет
          // маршрута загрузки»: воркер задеплоен старой версией. Без этой
          // подсказки оператор видит «Не найдено» и идёт искать файл.
          if (res.status === 404) {
            throw new Error("Загрузка фотографий появится после обновления сервера");
          }
