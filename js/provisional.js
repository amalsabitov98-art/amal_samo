...
(function (global) {
   ...
   * Номера рейсов и время — с настоящих билетов Centrum Air (несколько
   * билетов на разные даты сходятся, поэтому берём их как рабочие для
   * всего сезона).
   *
   ...
   * 2. ОТЕЛИ И НОЧИ
   * ---------------
   * ПОДТВЕРЖДЕНО ВАУЧЕРАМИ ОПЕРАТОРА (Centrum Air / Turon Tourism):
   *   заезд через Батуми (BUS): Batumi View Luxury 4 ночи → Rhisos Gold
   *     Otel Rize 3 ночи;
   *   заезд через Трабзон (TZX): Rhisos Gold Otel Rize 4 ночи → Batumi
   *     View Luxury 3 ночи.
   * Разбивка 4 + 3 одинакова для всех заездов сезона.
   *
   * ЧТО УТОЧНИТЬ: только категория и вид номера (море/балкон) — их в
   * ваучерах нет, определяются при бронировании.
   */
  var HOTELS = {
    BUS: [
  ...
  global.TuronProvisional = {
    ...
    flightNoteHtml: function () {
      return '<p class="tt-confirmed"><strong>Подтверждено оператором:</strong> рейсы ' +
        SCHEDULE.weekday_label + ' по ' + SCHEDULE.season_end_label +
        ' включительно · багаж 23 кг · ручная кладь 8 кг. ' +
        'Номера и время рейсов — по билетам Centrum Air.</p>';
    },

    // Ночи по отелям подтверждены ваучерами оператора (4+3 по обоим
    // маршрутам). Плашка зелёная, а не жёлтая — данные финальные.
    hotelNoteHtml: function () {
      return '<p class="tt-confirmed"><strong>Подтверждено ваучерами:</strong> ' +
        'распределение ночей по отелям (4 + 3). Категории номеров и ' +
        'вид уточняются при бронировании.</p>';
    },
