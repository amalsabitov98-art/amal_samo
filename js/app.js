(function () {
  ...
  function voucherCss() {
    return (
      ...
      ".sheet{max-width:820px;margin:24px auto;background:" + V.paper + ";" +
        "border:1px solid " + V.line + ";border-radius:16px;padding:34px 38px 30px;" +
      ...
      ".vh-order{text-align:right;white-space:nowrap}" +
      ".vh-order span{display:block;font-size:13px;font-weight:700;color:" + V.forest + "}" +
      ".vh-order strong{display:block;font-size:30px;font-weight:800;color:" + V.forestDeep + ";line-height:1.1}" +
      ".vh-order em{display:block;font-style:normal;font-size:11px;color:" + V.muted + ";margin-top:3px}" +
      /* заголовки разделов */
      "h2{font-size:17px;font-weight:800;color:" + V.forest + ";margin:22px 0 10px}" +
      "h2:first-of-type{margin-top:0}" +
      /* карточки */
      ".card{border:1.5px solid " + V.line + ";border-radius:11px;background:#fff;padding:11px 14px}" +
      ".lbl{font-size:9.5px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:" + V.muted + "}" +
      ".val{font-size:15px;font-weight:700;color:" + V.forestDeep + "}" +
      /* пассажиры */
      ".pax{display:grid;grid-template-columns:1fr auto auto;gap:10px;margin-bottom:9px}" +
      ".pax-name{display:flex;align-items:center;gap:11px}" +
      ".pax-name .val{font-size:17px;letter-spacing:.01em}" +
      ".pax-ico{width:26px;height:26px;flex:none;fill:" + V.forest + "}" +
      ".pax-type{font-size:10px;font-weight:700;letter-spacing:.08em;color:" + V.gold + ";margin-top:2px}" +
      /* маршрут */
      ".route{display:grid;grid-template-columns:1fr 250px;gap:12px;align-items:start}" +
      ".legs{display:grid;gap:9px}" +
      ".leg{display:grid;grid-template-columns:1fr 42px 1fr;align-items:center;gap:0}" +
      ".leg .card{text-align:center;padding:9px 10px}" +
      ".leg-date{font-size:12px;color:" + V.ink + "}" +
      ".leg-time{font-size:12px;color:" + V.ink + ";margin-bottom:2px}" +
      ".leg-city{font-size:17px;font-weight:800;color:" + V.forestDeep + ";letter-spacing:.01em}" +
      ".leg-code{font-size:10px;letter-spacing:.06em;text-transform:uppercase;color:" + V.muted + ";margin-top:2px}" +
      ".leg-mid{display:flex;justify-content:center}" +
      ".leg-dot{width:32px;height:32px;border-radius:50%;background:" + V.forestSoft + ";" +
        "display:flex;align-items:center;justify-content:center}" +
      ".leg-dot svg{width:17px;height:17px;fill:#fff}" +
      ".facts{display:grid;grid-template-columns:1fr 1fr;gap:14px 10px;height:100%;align-content:center}" +
      ".facts .val{font-size:16px;color:" + V.forestSoft + "}" +
      /* отели */
      ".hotels{display:grid;grid-template-columns:1fr 1fr;gap:11px}" +
      ".hotel .val{font-size:15px}" +
      ".hotel-dates{font-size:11.5px;color:" + V.muted + ";margin-top:2px}" +
      /* услуги */
      ".svc{display:grid;gap:5px}" +
      ".svc div{border:1.5px solid " + V.line + ";border-radius:9px;background:#fff;padding:7px 13px;font-size:13px}" +
      /* подвал */
      ".vf{margin-top:22px;padding-top:14px;border-top:1px solid " + V.line + ";" +
        "display:flex;justify-content:space-between;gap:20px;font-size:11.5px;color:" + V.muted + "}" +
      ".vf strong{color:" + V.forest + "}" +
      ".warn{margin-top:12px;padding:9px 13px;border-radius:9px;background:#fbf3e3;" +
        "border:1px solid #e8d9ae;color:#8a6a18;font-size:11.5px}" +
      ".actions{max-width:820px;margin:0 auto 30px;display:flex;gap:10px}" +
  ...
  // Билет добавляет к общим стилям только своё: таблицу пассажиров и
  // сегменты перелёта. Всё остальное — те же карточки, что у ваучера.
  function ticketCss() {
    return (
      "table.tk{width:100%;border-collapse:collapse;margin:0 0 4px}" +
      "table.tk th{font-size:9.5px;font-weight:700;letter-spacing:.09em;text-transform:uppercase;" +
        "color:" + V.muted + ";text-align:left;padding:0 10px 6px;border-bottom:1.5px solid " + V.line + "}" +
      "table.tk td{padding:9px 10px;border-bottom:1px solid " + V.line + ";font-size:13.5px}" +
      "table.tk td strong{color:" + V.forestDeep + ";font-size:14.5px}" +
      "table.tk tr:last-child td{border-bottom:0}" +
      ".seg{border:1.5px solid " + V.line + ";border-radius:11px;background:#fff;margin-bottom:10px;overflow:hidden}" +
      ".seg-head{display:flex;justify-content:space-between;gap:14px;padding:8px 15px;" +
        "background:" + V.ivory + ";border-bottom:1.5px solid " + V.line + ";" +
        "font-size:11.5px;font-weight:700;color:" + V.forest + ";letter-spacing:.02em}" +
      ".seg-head span:last-child{font-weight:600;color:" + V.muted + "}" +
      ".seg-body{display:grid;grid-template-columns:1fr 120px 1fr;align-items:center;padding:13px 15px;gap:10px}" +
      ".seg-body .val{font-size:22px;line-height:1.1}" +
      ".seg-city{font-size:13px;font-weight:600;color:" + V.forest + ";margin-top:3px}" +
      ".seg-date{font-size:11.5px;color:" + V.muted + ";margin-top:1px}" +
      ".seg-mid{display:flex;flex-direction:column;align-items:center;gap:5px}" +
      ".seg-dur{font-size:11px;color:" + V.muted + ";white-space:nowrap}"
    );
  }
  ...
  function legHtml(leg, date) {
    return (
      '<div class="leg">' +
        ...
        "</div>" +
        '<div class="leg-mid"><span class="leg-dot">' + PLANE_SVG + "</span></div>" +
        '<div class="card">' +
          '<div class="leg-date">' + esc(vDate(date)) + "</div>" +
  ...
  function openVoucher(code) {
    ...
    var hotels = TuronProvisional.hotelsFor({ transport: b.transport });
    ...
    win.document.write(
      ...
      '<div class="sheet">' +
        ...
        (hotelHtml ? "<h2>Проживание:</h2><div class=\"hotels\">" + hotelHtml + "</div>" : "") +

        "<h2>Заезд:</h2>" +
        '<div class="svc"><div>' + esc(formatRange(b.date_start, nights)) + " · " +
          esc(TRANSPORT[b.transport] || b.transport) + " · заезд " +
          esc(b.departure_code) + "</div>" +
          (b.note ? "<div>Примечание: " + esc(b.note) + "</div>" : "") +
        "</div>" +

        '<div class="vf">' +
          "<div><strong>" + esc(op.name) + "</strong><br>" + esc(op.address) + "</div>" +
          "<div style=\"text-align:right\"><strong>" + esc(op.contact_name) + "</strong> · " +
            esc(op.phone) + "<br>" + esc(op.email) + "</div>" +
        "</div>" +
        '<div class="warn">Бланк предварительный — форму ваучера оператор ещё ' +
          "не утвердил. Рейсы и отели указаны по подтверждённому расписанию сезона.</div>" +
      "</div>" +
