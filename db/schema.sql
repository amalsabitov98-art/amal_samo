...
CREATE TABLE IF NOT EXISTS tours (
  ...
  -- Когда тур впервые открыли в продажу. NULL = черновик, то есть он ещё
  -- ни разу не публиковался и виден только оператору. Отдельного поля
  -- статуса нет намеренно: «черновик» и «снят с продажи» — это
  -- is_bookable = 0 в обоих случаях, и различает их ровно факт публикации.
  -- Второе поле статуса рано или поздно разъехалось бы с is_bookable.
  published_at         TEXT,
  note                 TEXT,
  -- контент карточки тура в каталоге (виден гостю без входа)
  description          TEXT,
  -- Строка над названием на публичной карточке («Загадочный Карадениз»).
  -- Раньше была зашита в js/catalog.js через destination === "Умра", и
  -- ЛЮБОЙ тур не из умры выходил подписанным Караденизом.
  kicker               TEXT,
  nights               INTEGER,
  ...
  from_price           INTEGER,
  -- Заглавная фотография карточки тура. Раньше фон карточки был зашит в
  -- коде под каждый продукт (видео у Карадениза, статичный кадр у умры), и
  -- у тура, заведённого оператором, фона не было вовсе. Ссылка на файл в
  -- хранилище, а не сам файл: картинка в базе раздула бы каждый ответ
  -- каталога, а её всё равно отдаёт отдельный маршрут с длинным кэшем.
  hero_image           TEXT,
